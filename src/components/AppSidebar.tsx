import { LayoutDashboard, BarChart3, Settings, LogOut, User, CalendarDays } from "lucide-react";
    ...
    <Sidebar collapsible="icon">
      <SidebarHeader className="p-4">
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div>
              <h1 className="font-display font-bold text-lg text-sidebar-foreground tracking-tight">KevQuest</h1>
      ...
      <SidebarFooter className="p-3 border-t border-sidebar-border">
        {!collapsed && user && <p className="text-[11px] text-sidebar-foreground/60 truncate mb-2 px-1">{user.email}</p>}
        <Button variant="ghost" size={collapsed ? "icon" : "sm"} className="w-full text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-sidebar-accent" onClick={handleLogout}>
